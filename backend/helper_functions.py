...
def parse_flight_data(data):
    ...
    for route in json_data:
        if len(route["legs"]) == 1:
            result["flights"].append({"duration":route["legs"][0]["durationInMinutes"], "price":route["price"]["formatted"], "flt":route["legs"][0]["segments"][0]["flightNumber"], "airline":route["legs"][0]["carriers"]["marketing"][0]["name"], "logo":route["legs"][0]["carriers"]["marketing"][0]["logoUrl"], "origin":route["legs"][0]["origin"]["displayCode"], "dest":route["legs"][0]["destination"]["displayCode"]})
    return result
